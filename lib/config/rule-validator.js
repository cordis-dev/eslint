...
class RuleValidator {
	...
	validate(config) {
        ...
        // to prevent 'Node must be provided when reporting error if location is not provided' error
        const hasTsNoUnusedVarsRule = Object.prototype.hasOwnProperty.call(config.rules, '@typescript-eslint/no-unused-vars');

        if (hasTsNoUnusedVarsRule) {
            // Remove no-unused-vars rule if present
            if (Object.prototype.hasOwnProperty.call(config.rules, 'no-unused-vars')) {
